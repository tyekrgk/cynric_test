...
class HTTPClient(object):
    ...
    >>>HTTPClient('htt://www.baidu.com').send()
    <Response [200]>
    '''

    def __init__(self,url,method='GET',headers=None,cookies=None):
        '''headers:字典。 例如：headers={'Content_type':'text/html'},cookies也是字典'''
        self.url = url
        self.session = requests.session()
        self.method = method.upper()
        if self.method not in METHODS:
            raise UnsupportMethodException('不支持的method:{0}，请检查传入的参数！'.format(self.method))
